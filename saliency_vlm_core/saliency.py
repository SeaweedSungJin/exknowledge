    ...
    def get_saliency_cropped_image(
        ...
        for y in tqdm(range(0, H - window_size + 1, stride), desc="Analyzing Patches"):
            for x in range(0, W - window_size + 1, stride):
                ...
                with torch.no_grad():
                    inputs = self.processor(images=[patch_pil], return_tensors="pt").to(self.device)
                ...
                patch_similarities.append({"similarity": sim, "center": (x + window_size // 2, y + window_size // 2)})
        ...
        for p in patch_similarities:
            ...
            heatmap[y_start:y_start+window_size, x_start:x_start+window_size] += p["similarity"]
            count_map[y_start:y_start+window_size, x_start:x_start+window_size] += 1
        ...
        if rows.size > 0:
            ...
            print(f"중요 영역 BBox:({x_min}, {y_min}, {x_max}, {y_max}) 로 이미지를 잘랐습니다.")
        else:
            final_image = image
